    ...
    >
      ...
      <div className="w-full text-start cursor-pointer">
        <div className="flex justify-between border-b-[1.3px] mb-2 pb-2">
          {/* Left section */}
          <div className="">
            <h1 className="font-medium text-md">{order.id}</h1> {/* order id*/}
            <h2 className="text-gray-600">{formatDate}</h2> {/* order date*/}
            <p className="text-gray-600 text-sm">{order.propertyType}</p>{' '}
            {/* order property type*/}
          </div>
        ...
        <div className="flex items-center text-sm text-gray-500 mr-4">
          <span className="w-56 md:w-80">{order.problems.join(', ')}</span>
